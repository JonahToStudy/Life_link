//4.是否已進行登入處理       
  function statusChangeCallback(response) {  // Called with the results from FB.getLoginStatus().
    ...
    if (response.status === 'connected') {   // 判斷是否已登入fb及本網站
      testAPI();  //已登入則呼叫testAPI()函數
    } else {                                 // Not logged into your webpage or we are unable to tell.
      document.getElementById('status').innerHTML = '請使用FB帳號登入本站!';
    }
  }
 ...
 //5.已登入處理
  function testAPI() {                      // Testing Graph API after login.  See statusChangeCallback() for when this call is made.
    ...
    FB.api('/me?fields=id,name,email,last_name,picture', function(response) {
      ...
      document.getElementById('demo3').innerHTML="您的電郵: " + response.email;
      ...
      image.src=response.picture.data.url;  //將大頭貼的網址指定給<img>的來源
      document.querySelector('#fbimg').appendChild(image);   //將圖片置於<div>內
      
      document.getElementById('fbimg').style.backgroundColor="tomato";
    });
  }
